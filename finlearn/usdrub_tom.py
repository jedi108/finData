...
# Создадим ещё один набор данных, только в каждой i-й строке вместо цены закрытия pi будем хранить изменение этой цены по сравнению с предыдущим днём, 
# выраженное в процентах по отношению к предыдущему дню, по формуле
...
# Добавим к новому набору данных те же 2 столбца для задержанных данных, но вместо цены закрытия будем хранить процентное изменение. 
# Кроме того, нам потребуется ещё один столбец Direction (Направление): если цена закрытия выросла по сравнению с предыдущим днём, то в этот столбец запишем 1, а если цена закрытия уменьшилась, то значение -1.
for i in range(0, count):
